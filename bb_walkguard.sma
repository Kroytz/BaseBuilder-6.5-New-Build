...
#define AUTHOR "mogel"
#define TASK_NUKE 9987
#define TASK_AMBIENCE 1234
...
new g_touched[33]
new g_nukelaunched = false
new g_firsttouch
new g_semiclipoffed = false
new g_closed = false
...
new spr_dot		// ben飆igt f die Lininen

new editor = 0		// 该球员目前上创建..男人?verkraftet nur einen Editor

new camperzone[33]		// 最后一条消息露营区
new Float:campertime[33]	// 到达的区域中的第一次约会
new Float:camping[33]		// 露营的最后日期
...
new slap_botdamage
new admin_immunity
new icon_damage		// 损伤图标
new bool:no_change_bgm = false
...
new amblist[][] = {
"sound/zmParadise/ze_showdown.mp3",
"sound/zmParadise/ze_rescue2.mp3"
}
...
public plugin_init() {
	...
	pcv_botdamage = register_cvar("wg_botdamage", "0")	// 机器人只是普通的愚蠢......也看不懂
	pcv_immunity = register_cvar("wg_immunity", "0")		// 管理员免疫力酹不需要
	pcv_direction = register_cvar("wg_direction", "1")	// 训练鄟谎言对掴
	pcv_botdirection = register_cvar("wg_botdirection", "1") // dito f Bots
	pcv_damageicon = register_cvar("wg_damageicon", "262144") // 实际上箭头
	...
	register_srvcmd("zp_no_change_bgm", "no_ch_bgm")
...
public client_disconnect(player) {
	// aus irgend welchen Grden ist der Spieler einfach wech........
...
public Event_RoundStart() {
	...
	g_nukelaunched = false
	g_semiclipoffed = false
	g_closed = false
	for(new i = 0; i<33; i++)
	{
	...
	// 执行配置文件 (zombieplague.cfg)
	new mapname[64]
	get_mapname(mapname, 63)

	if(containi(mapname, "ze_") == 0)
	{
		server_cmd("exec %s/zombieescape.cfg", cfgdir)
	}
	else
	{
...
public ZoneTouch(player, zone) {
	...
	if( (ZONEMODE:zm == ZM_KILL_T1) && !g_closed )
	{
		g_closed = true
		g_semiclipoffed = true
		server_cmd("semiclip_offnow")
	}

	if((ZONEMODE:zm == ZM_CAMPING) && (!g_semiclipoffed))
	{
		g_semiclipoffed = true
		...
		// 执行配置文件 (zombieplague.cfg)
		server_cmd("exec %s/zp_norespawn.cfg", cfgdir)

		if(!g_closed)
		{
			ColorChat(0, GREY, "Console: ** SOMEONE REACHED RESCUE ZONE. SEMICLIP DISABLED **")
			server_cmd("sayserver @** 到達救援地點，人穿人功能已經關閉 **")
		
			if(!no_change_bgm)
			{
				// 停止目前的氛围和玩一个新的
				server_cmd("zp_stop_amb")
				set_task(1.0, "startamb", TASK_AMBIENCE)
			}
		}
	...
	{
		g_touched[player] = true

		if(!g_nukelaunched)
		{
			ColorChat(0, GREY, "Console: ** NUCLEAR IN 5 SECONDS **")
			server_cmd("sayserver @** 核弹将在 5 秒后入境 **")

			g_firsttouch = zp_get_user_zombie(player)
			g_nukelaunched = true
			set_task(5.0, "nukebomb", TASK_NUKE, _, _, "b")
		}

	}
	
	// 露营
	if ( ((ZONEMODE:zm == ZM_CAMPING_T1) && (userteam == 1)) || ((ZONEMODE:zm == ZM_CAMPING_T2) && (userteam == 2)) ) {
		if (!camping[player]) {
...
public startamb(taskid)
{
	client_cmd(0, "mp3 play %s", amblist[random_num(0, sizeof amblist - 1)])
}

public nukebomb(TASKID)
{
	for(new id = 0; id<33; id++)
	{
		if(is_user_connected(id) && is_user_alive(id))
			if(zp_get_user_zombie(id) != g_firsttouch)
				user_silentkill(id)
...
public ZoneModeCamper(player) {
	...
	{
		// so ein Feigling ... hat sich einfach verdrkt ^^
...
public LoadWGZ() {
	...
	while( (line = read_file(zonefile , line , input , 127 , len) ) != 0 ) 
	{
		...
		new Float:mins[3], Float:maxs[3], Float:pos[3]	// Gr廲e & Position
		...
		{
			// 瓏derungen von CS:CZ zu allen Mods
...
public ShowZoneBox(entity) {
	...
	// Distanz zum Treffer ... ist Wert gr廲er dann war da etwas
...
public HideAllZones() {
	editor = 0	// Men?f den n踄hsten wieder frei geben ... ufnktionalit酹 aktivieren
	for(new i = 0; i < maxzones; i++)
	{
